...
"Usage: ndsend <address> <interface> <num_times> (example: ndsend 2001:DB8::1 eth0 1)\n\n"
...
int main(int argc, char** argv)
{
	...
	if (inet_pton(AF_INET6, argv[1], &src_ipaddr) <= 0)
		return -1;
	iface = argv[2];
        send_count = strtol(argv[3],NULL,10);
	...
	for(iter=0; iter<send_count; iter++)
	{
		sender();
		sleep(1);
	}
